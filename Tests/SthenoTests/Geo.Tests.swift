...
struct GeoTests {
    @Test func `Great-circle distance between the same point is zero`() async throws {
        let a = Coordinate(latitude: 0, longitude: 0)
        #expect(Geo.distance(from: a, to: a) == 0)
    }

    @Test func `Distance between two known points (London-Paris)`() async throws {
	...
	@Test func `totalDistance with a linear path`() async throws {
		let points = [
	...
	@Test func `totalDistance with one single fix`() async throws {
		let points = [
			Coordinate(latitude: 0, longitude: 0),
		]
		let dist = Geo.totalDistance(points: points)
    ...
    @Test func `initialBearing returns North for due north`() async throws {
    ...
    @Test func `initialBearing returns East for due east`() async throws {
        let from = Coordinate(latitude: 0, longitude: 0)
