...
// `DateFormatter` formatting traps (`unreachable`) on WASI's
// swift-corelibs-foundation, so these date-display tests are excluded there;
// the coverage metric is produced on the Linux runner, which keeps them.
#if !os(WASI)
