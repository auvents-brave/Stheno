...
@testable import Stheno
	...
	@Test func `Create Date from ISO string`() async throws {
		...
		let withoutNano = Calendar.current.date(
			from: DateComponents(
		...
		let withNano = Calendar.current.date(
			from: DateComponents(
				timeZone: TimeZone(abbreviation: "GMT"),
				year: 2025,
				month: 8,
				...
				minute: 29,
				second: 23,
				nanosecond: 321_000_000))

		let nano = Calendar.current.component(.nanosecond, from: d)
		#expect(round(Double(nano) / 1000000.0) == 321)

		#expect(
			round(Double(nano) / 1000000.0)
				== round(Double(Calendar.current.component(.nanosecond, from: withNano!)) / 1000000.0))
