...
struct BonjourDefaultServiceTypesTests {
	...
	@Test func `entries are unique and complete`() {
		#expect(bonjourDefaultServiceTypes.count == 28)
		// No duplicate labels slipped in as the list grew.
		let labels = Set(bonjourDefaultServiceTypes.map(\.label))
		#expect(labels.count == bonjourDefaultServiceTypes.count)
	}
	...
	@Test func `all bonjourTypes end with _tcp`() {
		for entry in bonjourDefaultServiceTypes {
			#expect(
				entry.bonjourType.hasSuffix("._tcp"),
...
//                         immediately; tests accept both outcomes.
	...
	struct BonjourDiscoveryLinuxLifecycleTests {
		...
		@Test func `multiple independent browse sessions do not interfere`() async throws {
			...
			let d2 = BonjourDiscovery()
			async let s1: Void = {
				do { for try await _ in d1.browse(serviceTypes: [], timeout: 0.05) {} } catch is BonjourDiscoveryError {
				}
			}()
			async let s2: Void = {
				do { for try await _ in d2.browse(serviceTypes: [], timeout: 0.05) {} } catch is BonjourDiscoveryError {
				}
			}()
