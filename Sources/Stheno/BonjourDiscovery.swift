...
/// Default marine mDNS service types browsed by ``BonjourDiscovery``.
///
/// The first three are well-documented standards; the vendor entries are
/// best-effort — add confirmed types from `dns-sd -B` output.
public let bonjourDefaultServiceTypes: [BonjourServiceEntry] = [
    ...
    // Vendor-specific (unverified — harmless if the device doesn't advertise them)
...
fileprivate func resolveEndpoint(
    ...
    let path: String = {
        if case let .bonjour(txt) = result.metadata,
           let p = txt["path"], !p.isEmpty { return p }
    ...
    connection.stateUpdateHandler = { state in
        ...
        case .ready:
            if let cp = connection.currentPath,
               case let .hostPort(h, port) = cp.remoteEndpoint {
                let raw: String
                switch h {
                case let .name(n, _): raw = n
                case let .ipv4(a):    raw = "\(a)"
                case let .ipv6(a):    raw = "\(a)"
                ...
                continuation.yield(DiscoveredEndpoint(
...
fileprivate final class WinBrowseCtx: @unchecked Sendable {
    ...
    init(entry: BonjourServiceEntry,
         continuation: AsyncThrowingStream<DiscoveredEndpoint, any Error>.Continuation) {
        self.entry = entry
...
fileprivate final class WinResolveCtx: @unchecked Sendable {
    let name: String
    let entry: BonjourServiceEntry
    let continuation: AsyncThrowingStream<DiscoveredEndpoint, any Error>.Continuation
    var cancel = DNS_SERVICE_CANCEL()
    init(name: String, entry: BonjourServiceEntry,
         continuation: AsyncThrowingStream<DiscoveredEndpoint, any Error>.Continuation) {
        self.name = name
        self.entry = entry
        self.continuation = continuation
    ...
    while let rec = cur {
        if rec.pointee.wType == DNS_TYPE_PTR,
           let nameHost = rec.pointee.Data.PTR.pNameHost {
            ...
            startWinResolve(name: instanceName,
    ...
    public func browse(
        ...
        AsyncThrowingStream { continuation in
            ...
            for entry in serviceTypes {
                let queryName = entry.bonjourType.hasSuffix(".local")
                    ? entry.bonjourType
...
fileprivate enum AvahiLoader {
    ...
    private static func load() -> AvahiAPI? {
        ...
        guard
            let f0: PFN_DNSServiceCreateConnection  = sym("DNSServiceCreateConnection",  as: PFN_DNSServiceCreateConnection.self),
            ...
            let f3: PFN_DNSServiceProcessResult      = sym("DNSServiceProcessResult",      as: PFN_DNSServiceProcessResult.self),
            let f4: PFN_DNSServiceRefDeallocate      = sym("DNSServiceRefDeallocate",      as: PFN_DNSServiceRefDeallocate.self),
        ...
        return AvahiAPI(createConnection: f0, browse: f1, resolve: f2,
...
fileprivate final class BrowseCtx: @unchecked Sendable {
    let entry: BonjourServiceEntry
    let mainRef: DNSServiceRef
    let continuation: AsyncThrowingStream<DiscoveredEndpoint, any Error>.Continuation
    init(entry: BonjourServiceEntry, mainRef: DNSServiceRef,
         continuation: AsyncThrowingStream<DiscoveredEndpoint, any Error>.Continuation) {
        self.entry = entry; self.mainRef = mainRef; self.continuation = continuation
    }
}

fileprivate final class ResolveCtx: @unchecked Sendable {
    let name: String
    let entry: BonjourServiceEntry
    let continuation: AsyncThrowingStream<DiscoveredEndpoint, any Error>.Continuation
    init(name: String, entry: BonjourServiceEntry,
         continuation: AsyncThrowingStream<DiscoveredEndpoint, any Error>.Continuation) {
        self.name = name; self.entry = entry; self.continuation = continuation
    }
}

// MARK: C callbacks

fileprivate let dnsBrowseReply: DNSServiceBrowseReply = {
    ...
    let resolveCtx = ResolveCtx(name: name, entry: browseCtx.entry,
                                continuation: browseCtx.continuation)
    let resolvePtr = Unmanaged.passRetained(resolveCtx).toOpaque()
    var subRef: DNSServiceRef? = browseCtx.mainRef
    let err = api.resolve(&subRef, DNSServiceFlags(kDNSServiceFlagsShareConnection),
...
fileprivate let dnsResolveReply: DNSServiceResolveReply = {
    ...
    public func browse(
        ...
        AsyncThrowingStream { [self] continuation in
            ...
            for entry in serviceTypes {
                ...
                let err = api.browse(&subRef, DNSServiceFlags(kDNSServiceFlagsShareConnection),
