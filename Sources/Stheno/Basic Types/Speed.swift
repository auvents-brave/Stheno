...
public struct Speed: Equatable, Sendable {
	/// The numeric speed, expressed in ``unit``.
	...
	public func formatted(as format: Format) -> Formatted {
		switch format {
		case .kilometersPerHour(let d): render(.kilometersPerHour, d)
		case .milesPerHour(let d): render(.milesPerHour, d)
		case .knots(let d): render(.knots, d)
