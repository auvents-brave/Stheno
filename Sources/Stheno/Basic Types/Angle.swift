...
/// An angle in degrees normalised to the [0, 360) range.
public struct Angle {
    ...
    /// A strategy for rendering an angle as a direction value.
    public protocol DirectionFormat {
        /// The type produced by the format.
        associatedtype Output
        /// Formats the given angle into the format's output.
        static func format(_ value: Angle) -> Output
    }

    /// Formats an angle as its numeric value paired with the degree symbol.
    ...
    /// Formats an angle as a cardinal direction string, when one applies.
    ...
    #Playground {
        ...
        // Initialise with cardinal direction
