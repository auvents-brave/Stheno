...
public struct Distance: Equatable, Sendable {
    ...
    public func formatted(as format: Format) -> Formatted {
        switch format {
        case let .meters(d): render(.meters, d)
        case let .feet(d): render(.feet, d)
        case let .kilometers(d): render(.kilometers, d)
        case let .miles(d): render(.miles, d)
        case let .nauticalMiles(d): render(.nauticalMiles, d)
        case .adaptiveMetric:
