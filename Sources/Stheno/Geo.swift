...
public struct Geo {
  ...
  /// Calculates the total distance (in kilometres) along a series of geographic coordinates.
  ///
  /// - Parameter points: An ordered array of coordinates representing the path.
  /// - Returns: The total distance in kilometres by summing the distances between each consecutive pair of points.
  ...
  public static func initialBearing(from: Coordinate, to: Coordinate) -> Double {
    ...
    let x =
      cos(from.latitudeRadians) * sin(to.latitudeRadians)
      - sin(from.latitudeRadians) * cos(to.latitudeRadians)
      * cos(to.longitudeRadians - from.longitudeRadians)
