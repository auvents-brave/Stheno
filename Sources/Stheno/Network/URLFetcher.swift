  ...
  /// >  Use only the pure Toolchain, not [SwiftNIO](https://github.com/apple/swift-nio), even on Linux or Windows.
  ///
  ...
  ///   decoding may fail.
  public func downloadURLasString(
    from url: URL, completion: @escaping @Sendable (Result<String, any Error>) -> Void
  ) {
    ...
    let task = requestSession.dataTask(with: url) { data, response, error in
      ...
      if let error = error {
        Logger(label: "Networking").error(
          "Request failed with transport error.", metadata: ["error": "\(error)"])
      ...
      guard let httpResponse = response as? HTTPURLResponse,
        ...
        Logger(label: "Networking").error(
          ...
          metadata: ["statusCode": "\(httpResponse?.statusCode ?? -1)"])
      ...
      guard let data = data,
        ...
        Logger(label: "Networking").error(
          "Failed to decode response body as UTF-8 string.",
