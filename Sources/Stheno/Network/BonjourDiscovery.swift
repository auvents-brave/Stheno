...
/// A Bonjour/mDNS service type to browse, paired with the URL scheme, base path
/// and label applied to every ``DiscoveredEndpoint`` resolved from it.
...
public let bonjourDefaultServiceTypes: [BonjourServiceEntry] = [
  // Marine — standards
  .init(
    bonjourType: "_signalk-http._tcp", scheme: "http", defaultPath: "/signalk",
    label: "signalk-http"),
  .init(
    bonjourType: "_signalk-ws._tcp", scheme: "ws", defaultPath: "/signalk", label: "signalk-ws"),
  ...
  public final class BonjourDiscovery: @unchecked Sendable {
    ...
    private static func makeBrowser(
      ...
      let descriptor = NWBrowser.Descriptor.bonjourWithTXTRecord(
        type: entry.bonjourType, domain: nil)
  ...
  private enum AvahiLoader {
    ...
    private static func load() -> AvahiAPI? {
      ...
      guard
        ...
        let f5: PFN_DNSServiceRefSockFD = sym(
          "DNSServiceRefSockFD", as: PFN_DNSServiceRefSockFD.self),
        let f6: PFN_TXTRecordGetValuePtr = sym(
          "TXTRecordGetValuePtr", as: PFN_TXTRecordGetValuePtr.self)
      else { return nil }
      return AvahiAPI(
