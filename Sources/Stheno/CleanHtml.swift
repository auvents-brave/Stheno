...
///
/// This function is intended to sanitise and clean up raw HTML content by:
...
///
/// - Parameter text: The input string containing HTML content to be sanitised. This can be either a raw HTML source or a text fragment with embedded HTML.
/// - Returns: A plain, human-readable string with all recognised HTML tags removed and all supported HTML entities decoded. If the input contains only safe, non-HTML content, the original string is returned.
///
/// ## Example
/// ```swift
/// let plain = cleanHtml(from: "<p>Copyright &copy; 2025</p>")
/// print(plain)
/// ```
/// Output:
/// ```
...
public func cleanHtml(from text: String) -> String {
  ...
  let htmlTags = [
    // Core and block tags
    "html", "head", "body", "div", "span", "p", "a", "img", "br", "hr", "h1", "h2", "h3", "h4",
    "h5", "h6",
    "ul", "ol", "li", "table", "tr", "td", "th", "form", "input", "button", "script", "style",
    "link", "meta",
    "title", "footer", "header", "nav", "section", "article", "aside", "main",
    // Inline and formatting
    "b", "strong", "em", "i", "u", "mark", "small", "del", "ins", "sub", "sup", "code", "pre", "s",
    "abbr", "cite",
    "dfn", "kbd", "samp", "var", "q", "blockquote", "address", "time", "progress", "meter", "wbr",
    "details",
    // Media
    "audio", "video", "source", "track", "canvas", "map", "area", "svg", "iframe", "object",
    "embed", "param",
  ...
  let tagStripped = regex.stringByReplacingMatches(
...
private func decodeHtmlEntities(in text: String) -> String {
  ...
  let entities: [String: String] = [
    ...
    "&Sacute;": "Ś", "&sacute;": "ś", "&Zacute;": "Ź", "&zacute;": "ź", "&Scaron;": "Š",
    "&scaron;": "š",
    "&Zcaron;": "Ž", "&zcaron;": "ž", "&Ccaron;": "Č", "&ccaron;": "č", "&Ecaron;": "Ě",
    "&ecaron;": "ě",
    "&Dcaron;": "Ď", "&dcaron;": "ď", "&Ncaron;": "Ň", "&ncaron;": "ň", "&Rcaron;": "Ř",
    "&rcaron;": "ř",
    "&Tcaron;": "Ť", "&tcaron;": "ť", "&Uring;": "Ů", "&uring;": "ů", "&Lacute;": "Ĺ",
    "&lacute;": "ĺ",
    "&Lcaron;": "Ľ", "&lcaron;": "ľ",
    // Misc
