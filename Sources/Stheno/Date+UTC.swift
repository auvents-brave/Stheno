...
extension Date {
	...
	/// Initialises a `Date` from an ISO 8601 formatted string, accepting optional fractional seconds.
	...
	init(fromISO: String) {
		...
		guard let date = formatter.date(from: fromISO) else {
			Logger(label: "Date+UTC").error(
				"Failed to parse ISO 8601 date string.", metadata: ["fromISO": "\(fromISO)"])
			return
		}
		self = date
	}
	...
	/// - SeeAlso: `display(displayAs:formatter:)`
